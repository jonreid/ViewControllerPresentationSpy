...
- Completion handler passed to `present(_:animated:completion:)` is no longer executed right away. Instead, it's captured in `capturedCompletion`. This way tests can execute it after executing a button action.
